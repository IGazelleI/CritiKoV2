                            ...
                            <div class="btn-group dropend">
                                ...
                                <ul class="dropdown-menu">
                                    @foreach($block as $b)
                                        ...
                                        <li>
                                            <a href="{{route('pdf.view', [$perSelected == null? $periods->first()->id : $perSelected, 4, $faculty->id, $klase->subject_id])}}" 
                                                class="dropdown-item {{$faculty->evaluated->where('period_id', isset($perSelected)? $perSelected : $periods->first()->id)->where('evaluatee', $faculty->user_id)->whereIn('evaluator', $students)->where('subject_id', $klase->subject->id)->isEmpty()? 'disabled' : ''}}" target="_blank"
                                            >
                            ...
                            <div class="btn-group dropend">
                                ...
                                <ul class="dropdown-menu">
                                    @foreach($block as $b)
                                        @foreach($b->klases->where('instructor', $faculty->user_id) as $klase)
                                        <li>
                                            <a href="{{route('pdf.view', [$perSelected == null? $periods->first()->id : $perSelected, 3, $faculty->id, $klase->subject_id])}}" 
                                                class="dropdown-item {{$faculty->evaluated->where('period_id', isset($perSelected)? $perSelected : $periods->first()->id)->where('evaluatee', $faculty->user_id)->whereIn('evaluator', $cofac)->where('subject_id', $klase->subject->id)->isEmpty()? 'disabled' : ''}}" target="_blank"
                                            >
                                                {{$klase->subject->descriptive_title}}
                                            </a>
                                        </li>
