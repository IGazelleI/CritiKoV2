<x-layout>
    <x-general-card>
        ...
        <div class="row">
            <div class="col">
                <form action="{{route('user.assignAssociateProcess')}}" method="POST">
                    @csrf
                        ...
                        <div class="col-8">
                            <div class="form-outline mb-4">
                                ...
                                <p class="text-secondary" style="font-size: 15px"> 
                                    Select Faculty member to be assigned as the College Associate Dean 
                                </p>
                                <select class="select form-select rounded-pill" name="user_id" onchange="console.log(value)">
                                    ...
                                    @unless ($faculty->isEmpty())
                                        @foreach($faculty as $det)
                                            <option value="{{$det->id}}" {{$det->isAssDean? 'selected' : ''}}> {{$det->fullName(true)}} </option>
                                        @endforeach
                                    @else
