<x-layout>
    <x-general-card>
        @if(isset($period))
            @if(isset($period->beginEnroll))
                @if($period->endEnroll < NOW()->format('Y-m-d') && !isset($enrollment))
                <h3 class="text-center m-4 bg-light p-4 rounded text-uppercase"> Enrollment ended on {{date('F d, Y @ D', strToTime($period->endEnroll))}} </h3>
                @elseif($period->beginEnroll <= NOW()->format('Y-m-d') || isset($enrollment))
                    <div class="row">
                        ...
                        <div class="col">
                            <div class="row m-4">
                                <div class="col-3">
                                    ...
                                    <input type="text" name="id_number" id="id_number" class="form-control rounded-pill" value="{{auth()->user()->students->first()->id_number}}" disabled/>
                                </div>
                                <div class="col-6">
                                    <label class="form-label ms-2">Name</label>
                                    <input type="text" class="form-control rounded-pill" placeholder="Name" value="{{auth()->user()->students->first()->fullName(false)}}" disabled/>
                                </div>
                            </div>
                ...
                @else
                <h3 class="text-center m-4 bg-light p-4 rounded text-uppercase"> Enrollment will open on {{date('F d, Y @ D', strToTime($period->beginEnroll))}} </h3>
                @endif
            @else
            <h3 class="text-center m-4 bg-light p-4 rounded text-uppercase"> Enrollment date not set </h3>
            @endif
        @else
            <div class="row">
                <div class="col">
